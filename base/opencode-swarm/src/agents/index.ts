		...
		agents.map((agent) => {
			...
			if (baseAgentName === 'architect') {
				...
				if (baseAgentName === 'coder') {
					// Coder delegates to ECC specialists
					(sdkConfig.permission as Record<string, 'allow'>) = { task: 'allow' };
					...
					// Critic delegates to ECC planning specialists
					(sdkConfig.permission as Record<string, 'allow'>) = { task: 'allow' };
				} else if (baseAgentName === 'docs') {
					// Docs delegates to ECC doc specialists
					(sdkConfig.permission as Record<string, 'allow'>) = { task: 'allow' };
				} else if (baseAgentName === 'designer') {
					// Designer delegates to ECC design specialists
					(sdkConfig.permission as Record<string, 'allow'>) = { task: 'allow' };
				} else if (baseAgentName === 'test_engineer') {
					// Test Engineer delegates to ECC test specialists
					(sdkConfig.permission as Record<string, 'allow'>) = { task: 'allow' };
				} else if (baseAgentName === 'sme') {
					// SME delegates to ECC specialists
					(sdkConfig.permission as Record<string, 'allow'>) = { task: 'allow' };
				} else if (baseAgentName === 'curator_init') {
					// Curator (Init) delegates to ECC specialist
					(sdkConfig.permission as Record<string, 'allow'>) = { task: 'allow' };
				} else if (baseAgentName === 'curator_phase') {
					// Curator (Phase) delegates to ECC specialist
					(sdkConfig.permission as Record<string, 'allow'>) = { task: 'allow' };
				}
			}
