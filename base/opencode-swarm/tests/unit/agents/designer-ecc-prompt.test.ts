...
describe('Designer Prompt ECC Exposure Adversarial Tests', () => {
	...
	const eccReviewAgents = [
		'code_reviewer', 'security_reviewer', 'cpp_reviewer', 'go_reviewer', 
		'kotlin_reviewer', 'java_reviewer', 'rust_reviewer', 'python_reviewer', 
		'typescript_reviewer', 'csharp_reviewer', 'flutter_reviewer', 
		'database_reviewer', 'healthcare_reviewer', 'gan_evaluator', 'opensource_sanitizer'
	];
	const eccBuildAgents = [
		'build_error_resolver', 'cpp_build_resolver', 'go_build_resolver', 
		'kotlin_build_resolver', 'java_build_resolver', 'rust_build_resolver', 
		'pytorch_build_resolver', 'dart_build_resolver'
	];
	const eccPipelineAgents = [
		'tdd_guide', 'e2e_runner', 'refactor_cleaner', 'performance_optimizer', 
		'opensource_forker', 'opensource_packager'
	];
	const eccSupportAgents = [
		'planner', 'doc_updater', 'docs_lookup', 'harness_optimizer', 
		'loop_operator', 'chief_of_staff', 'gan_planner'
	];
